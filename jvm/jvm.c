...
void native_method_call(jvm_thread *thread, const char *class_name, const char *method_name, const char *desc) {
    if (strcmp(class_name, "org/lwjgl/opengl/GL11") == 0) {
        if (strcmp(method_name, "glClear") == 0) {
            s4 mask = thread->stack[0].i;
            js_gl_clear(mask);
        } else if (strcmp(method_name, "glClearColor") == 0) {
            js_gl_clear_color(thread->stack[0].f, thread->stack[1].f, thread->stack[2].f, thread->stack[3].f);
        } else if (strcmp(method_name, "glViewport") == 0) {
            js_gl_viewport(thread->stack[0].i, thread->stack[1].i, thread->stack[2].i, thread->stack[3].i);
        } else if (strcmp(method_name, "glEnable") == 0) {
            js_gl_enable(thread->stack[0].i);
        } else if (strcmp(method_name, "glDisable") == 0) {
            js_gl_disable(thread->stack[0].i);
        } else if (strcmp(method_name, "glBindTexture") == 0) {
            js_gl_bind_texture(thread->stack[0].i, thread->stack[1].i);
        } else if (strcmp(method_name, "glDrawArrays") == 0) {
            js_gl_draw_arrays(thread->stack[0].i, thread->stack[1].i, thread->stack[2].i);
        } else if (strcmp(method_name, "glGenTextures") == 0) {
            js_gl_gen_textures(thread->stack[0].i, (void *)(intptr_t)thread->stack[1].i);
        }
    } else if (strcmp(class_name, "org/lwjgl/opengl/Display") == 0) {
        if (strcmp(method_name, "create") == 0) {
            js_create_window(854, 480, "Minecraft Web");
        } else if (strcmp(method_name, "swapBuffers") == 0) {
            js_swap_buffers();
        } else if (strcmp(method_name, "isCloseRequested") == 0) {
            jvm_push_int(thread, js_is_close_requested());
        } else if (strcmp(method_name, "setDisplayMode") == 0) {
            js_set_display_mode(854, 480);
        }
    } else if (strcmp(class_name, "org/lwjgl/input/Keyboard") == 0) {
        if (strcmp(method_name, "isKeyDown") == 0) {
            jvm_push_int(thread, js_get_key_state(thread->stack[0].i));
        }
    } else if (strcmp(class_name, "org/lwjgl/input/Mouse") == 0) {
        if (strcmp(method_name, "isButtonDown") == 0) {
            jvm_push_int(thread, js_get_mouse_button(thread->stack[0].i));
        } else if (strcmp(method_name, "getX") == 0) {
            jvm_push_int(thread, js_get_mouse_x());
        } else if (strcmp(method_name, "getY") == 0) {
            jvm_push_int(thread, js_get_mouse_y());
        }
    } else if (strcmp(class_name, "java/lang/System") == 0) {
        if (strcmp(method_name, "currentTimeMillis") == 0) {
            jvm_push_long(thread, (s8)js_current_time_millis());
        } else if (strcmp(method_name, "nanoTime") == 0) {
            jvm_push_long(thread, (s8)(js_current_time_millis() * 1000000));
        }
    }
}
