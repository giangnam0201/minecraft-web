...
typedef struct jvm_class {
    u1 *data; s4 data_len;
    u2 minor_ver, major_ver;
    u2 cp_count; u1 *cp_raw;
    u2 access_flags;
    u2 this_class;
    char *name;
    char *super_name;
    ...
    s4 static_size; u1 *static_data;
    s4 inst_size;
    bool initialized;
    struct jvm_class *super;
    s4 index;
} jvm_class;
typedef struct jvm_field { u2 access; u2 name_idx; u2 desc_idx; s4 offset; jvm_class *cls; } jvm_field;
typedef struct jvm_method {
    u2 access; u2 name_idx; u2 desc_idx;
    u1 *code; s4 code_len; s4 max_stack; s4 max_locals;
    jvm_class *cls;
} jvm_method;
typedef struct jvm_frame { jvm_method *method; u1 *pc; struct jvm_value *locals; struct jvm_value *stack; s4 sp; jvm_object *this_obj; } jvm_frame;
typedef struct jvm_thread {
    s4 id;
    jvm_frame frames[JVM_MAX_FRAMES]; s4 frame_cnt;
    struct jvm_value stack[JVM_STACK_SIZE];
    struct jvm_value locals[JVM_STACK_SIZE];
    bool running;
    jvm_object *exception;
} jvm_thread;
typedef union { s4 i; s8 l; f4 f; f8 d; jvm_object *r; } jvm_value;
...
void jvm_resolve_methods(jvm_class *c);
